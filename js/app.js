...
function loadStoredCredentials() {
  const id  = localStorage.getItem('wcl_client_id')     ?? '';
  const sec = localStorage.getItem('wcl_client_secret') ?? '';
  document.getElementById('inp-client-id').value     = id;
  document.getElementById('inp-client-secret').value = sec;
...
function checkCredentialState() {
  const hasCreds = wcl.hasCredentials();
  document.getElementById('cred-status').className = hasCreds ? 'cred-ok' : 'cred-missing';
  document.getElementById('cred-status').title     = hasCreds ? 'API credentials set' : 'No API credentials - click to configure';
}
...
async function loadReport() {
  ...
  if (!input) return;
  if (!wcl.hasCredentials()) { showModal('modal-credentials'); return; }
...
async function selectFight(fightId) {
  ...
  try {
    ...
    const dpsTable = parseDPSTable(dpsTableRaw);
    const hpsTable = parseHPSTable(hpsTableRaw);
    const dtTable  = parseDamageTakenTable(dtTableRaw);
    const dpsGraph = parseGraph(dpsGraphRaw, 'DamageDone');
    const hpsGraph = parseGraph(hpsGraphRaw, 'HealingDone');
    const deaths   = parseDeaths(deathEvents);
    const casts    = parseCasts(castEvents);
    const cooldownUsages = extractCooldownUsages(casts, S.report.actors);

    S.fightData = { dpsTable, hpsTable, dtTable, dpsGraph, hpsGraph, deaths, casts, cooldownUsages };
...
function renderCurrentTab() {
  ...
  switch (S.activeTab) {
    ...
    case 'analysis': {
      ...
      renderMistakes(document.getElementById('tab-analysis'), mistakeData, S.roastMode);
      break;
    }
    case 'timeline': {
      const timelineEvents = buildTimelineEvents(deaths, casts, dtTable);
      renderRaidTimeline(document.getElementById('tab-timeline'), S.fight, deaths, cooldownUsages, timelineEvents);
      break;
    }
  }
}
...
function openPlayerDetail(name) {
  ...
  if (!player) return;
  const dpsEntry = S.fightData.dpsTable.find(e => e.name === name);
  const hpsEntry = S.fightData.hpsTable.find(e => e.name === name);
  const deaths   = S.fightData.deaths.filter(d => d.targetName === name);
  const casts    = S.fightData.casts.filter(c => c.sourceName === name);
  renderPlayerDetail(document.getElementById('player-detail-content'), player, dpsEntry, hpsEntry, deaths, casts);
...
function showSkeleton() {
  document.getElementById('tab-overview').innerHTML = '<div class="skeleton-block"></div><div class="skeleton-block"></div><div class="skeleton-block"></div>';
}
