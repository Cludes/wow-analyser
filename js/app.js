...
async function checkCredentialState() {
  // Probe config.json to see if a CI token exists
  try {
    ...
    if (resp.ok) {
      ...
      if (cfg.token) {
        ...
        el.textContent = 'CI Token Active';
        el.title = `Token generated by GitHub Actions on ${cfg.generated ?? 'unknown'}`;
        document.getElementById('btn-api-settings').title = 'Using GitHub Actions token - manual credentials not needed';
        return;
      }
    }
  } catch {}
  ...
  const el = document.getElementById('cred-status');
  el.className   = hasCreds ? 'cred-ok' : 'cred-missing';
  el.textContent = 'API';
  el.title       = hasCreds ? 'Manual API credentials set' : 'No API credentials - click to configure';
}
